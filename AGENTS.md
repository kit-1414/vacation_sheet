...
- в конфиге бэка в свойтве app.frontend.url задан корневой урл фронта. на этот урл выполдняется релирект при успешной авторизации 
...
#### Работа с полшьзоватетелями
   ...
   Пользователи добавляются 2 способоми
   1. при логине, если пользователя в системе еще нет, емайл имя и фамилия еруться из OAuthаттрибутов 
   2. На странице добавления нового
...
#### Страница "Список пользоватетей"
  Страница содержит список пользователей системы с аттрибутами
  Имеет кнопку дл яперехода на страницу "Добавлеине пользователя"  
  В списке пользователей имеет ссылку на страницу редактирования пользователя

#### Страница "Добавлеине пользователя"
  Страница добаляет нового пользователя

#### Страница "Релактирование пользователя"
   Страница редактирует аттрибуты существующего пользователя
...
#### Страница "Список проектов"
  Страница содержит список Проектов(Projects) в виде таблицы.
  Страница содержит возможность удалить выбранные проекты
  Страница содержит кнопку создания нового проекта

  В списке(талице) показываются аттрибуты проектов (и только они):
  - имя
  - время добавления
  - размер команды - числом - является ссылкой на страницу редактирования участников проекта
...
#### Страница "Назначить участников проекта"
   Страница позволяет добавить назначить (добавить -удалить) участников проекта.
   в списке пользователей показывыать емайл имя и фамилию.
   Список можно сортировать по 1 любой колонке
   Список можно фильтровать любой колонке (включением xthtp like %<pattern>%)
